...
const CATEGORY_COLORS: Record<string, string> = {
  Conference: "bg-peach-50/60 text-stone-700",
  ...
  Grant: "bg-peach-100/60 text-stone-700",
};
  ...
  const content = (
    <main className="flex-1 overflow-y-auto p-8">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-stone-900 mb-1">Startup Events & Deadlines</h1>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="flex flex-wrap gap-1.5 ml-auto items-center">
              ...
              <label className="flex items-center gap-1.5 cursor-pointer ml-2">
                ...
                />
                <span className="text-xs text-stone-600">Free only</span>
              </label>
            </div>
          </div>
        </div>
        ...
        {isLoading && (
          <div className="space-y-4">
            {Array(5).fill(0).map((_, i) => (
              <div key={i} className="glass rounded-2xl p-6 border border-peach-200/40 skeleton h-40" />
            ))}
          </div>
        )}
        ...
        {!isLoading && (
          <>
            <p className="text-xs text-stone-400 mb-4">{events.length} events found</p>
            <div className="space-y-4">
              {events.map((event) => {
                const catColor = CATEGORY_COLORS[event.category] || "bg-peach-100/60 text-stone-700";
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                            event.free ? "bg-green-50 text-green-600" : "bg-peach-50/40 text-stone-500"
                          }`}>
                            {event.free ? "Free" : event.ticket_range}
                          </span>
                          <span className="text-[10px] text-stone-400 flex items-center gap-0.5">
                            <span className={`w-1.5 h-1.5 rounded-full inline-block ${
                              event.mode.includes("Online") ? "bg-peach-400" :
                        ...
                        <p className="text-xs text-stone-500 leading-relaxed mb-3">{event.description}</p>

                        {/* Highlights */}
                        <div className="flex flex-wrap gap-1 mb-3">
                          {event.highlights.map((h) => (
                            <span key={h} className="text-[10px] bg-peach-50/40 text-stone-500 px-2 py-0.5 rounded-md border border-peach-200/40">
                              {h}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {event.audience.map((a) => (
                            <span key={a} className="text-[10px] text-peach-600 bg-peach-50/60 px-2 py-0.5 rounded-full">
                              {a}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="sm:w-52 shrink-0 flex flex-col gap-3">
                        <div className="bg-peach-50/50 rounded-xl p-3 space-y-2">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs font-medium text-stone-700">{event.event_date}</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-2">
                            <Clock className={`w-3.5 h-3.5 mt-0.5 shrink-0 ${isRolling ? "text-green-500" : "text-rose-400"}`} />
                            <div>
                              <p className="text-[10px] text-stone-400">Deadline</p>
                        ...
                        <a
                          ...
                          className="flex items-center justify-center gap-1.5 px-4 py-2 btn-coral rounded-xl text-xs font-medium transition-colors"
                        >
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center justify-center gap-1 text-xs text-stone-400 hover:text-stone-600"
                        >

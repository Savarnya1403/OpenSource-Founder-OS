    ...
    <main className="flex-1 overflow-y-auto p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-stone-900 mb-1">Jargon Buster</h1>
            <p className="text-stone-500 text-sm">200+ startup, VC, and compliance terms explained with India-specific context.</p>
          </div>
        </div>
            ...
            {ALPHABET.map((l) => (
              <button
                ...
                className={`w-8 h-8 rounded-lg text-xs font-semibold transition-colors ${
                  activeLetter === l
                    ? "btn-coral"
                    : "bg-white border border-peach-200/40 text-stone-500 hover:bg-peach-50/60"
                }`}
              >
                {l}
              </button>
            ...
            {letters.map((letter) => (
              <div key={letter}>
                <div className="flex items-center gap-3 mb-3">
                  <span className="w-8 h-8 rounded-lg bg-peach-50/60 border border-peach-200/60 text-peach-700 font-bold text-sm flex items-center justify-center">
                    {letter}
                  </span>
                  <div className="flex-1 h-px bg-peach-200/40" />
                </div>
                <div className="space-y-3">
                  {grouped[letter].map((t) => (
                    <div key={t.term} className="glass rounded-xl p-5 shadow-sm">
                      <div className="flex items-start justify-between mb-2">
                        <h3 className="font-semibold text-stone-900">{t.term}</h3>
                        {t.category && (
                          <span className="text-xs bg-peach-50/40 text-stone-500 border border-peach-200/40 px-2 py-0.5 rounded-full ml-3 shrink-0">
                            {t.category}
                          </span>
                        )}
                      </div>

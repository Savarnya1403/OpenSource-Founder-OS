    ...
    <main className="flex-1 overflow-y-auto p-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-stone-900 mb-1">Sector Playbooks</h1>
            <p className="text-stone-500 text-sm">Deep-dive guides on building in specific sectors in India.</p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {playbooks.map((pb) => (
              ...
              >
                <div className="w-12 h-12 rounded-xl bg-peach-50/60 border border-peach-200/40 flex items-center justify-center mb-4 text-2xl">
                  {pb.icon || SECTOR_ICONS[pb.sector] || "📖"}
                </div>
                <span className="text-xs bg-peach-50/60 text-stone-700 border border-peach-200/40 px-2 py-0.5 rounded-full w-fit mb-2">
                  {pb.sector}
                </span>
                <h3 className="font-semibold text-stone-900 mb-2">{pb.title}</h3>
                <p className="text-stone-400 text-xs leading-relaxed mb-4 flex-1">{pb.description}</p>
                {pb.reading_time && (
                  <div className="text-xs text-stone-400 mb-3">{pb.reading_time} read</div>
                )}

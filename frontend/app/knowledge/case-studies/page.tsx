    ...
    <main className="flex-1 overflow-y-auto p-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-6 flex items-center gap-3">
          <Link href="/knowledge" className="text-stone-400 hover:text-stone-600">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-stone-900 mb-1">Indian Startup Case Studies</h1>
            <p className="text-stone-500 text-sm">Real journeys, honest lessons, and the decisions that mattered.</p>
          </div>
        </div>
        ...
        <div className="glass rounded-2xl p-5 mb-6">
          <select
            ...
            className="px-3 py-2.5 border border-peach-200/60 rounded-lg text-sm bg-white/60 focus:outline-none focus:outline-none focus:border-peach-400"
          >
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 text-stone-400">No case studies found.</div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {studies.map((cs) => (
              ...
              >
                {cs.cover_image ? (
                  <div
                    className="w-full h-28 rounded-xl mb-4 bg-cover bg-center bg-peach-100/50"
                  ...
                  <div className="w-full h-28 rounded-xl mb-4 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
                    <span className="text-2xl font-bold text-stone-300">{cs.startup_name[0]}</span>
                  </div>
                )}

                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-stone-900">{cs.startup_name}</h3>
                  {cs.valuation && (
                ...
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs bg-peach-50/60 text-stone-700 border border-peach-200/40 px-2 py-0.5 rounded-full">
                    {cs.sector}
                  </span>
                  <span className="text-xs text-stone-400">{cs.year}</span>
                </div>

                <p className="text-xs text-stone-500 mb-3 flex-1 leading-relaxed">{cs.tagline}</p>

                {cs.founder && (
                  <div className="text-xs text-stone-400 mb-3">By {cs.founder}</div>
                )}

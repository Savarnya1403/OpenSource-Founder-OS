  ...
  const content = (
    <main className="flex-1 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-1">VC Directory</h1>
          <p className="text-gray-500 text-sm">
            {total} venture capital firms investing in Indian startups.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {vcs.map((vc) => (
              <div key={vc.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow flex flex-col">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-semibold text-gray-900 text-base">{vc.name}</h3>
                  {vc.website && (
                    <a href={vc.website} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-brand-600 ml-2 shrink-0">
                ...
                {vc.description && (
                  <p className="text-xs text-gray-500 mb-3 line-clamp-2">{vc.description}</p>
                )}

                {/* Stage badges */}
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {(vc.stages || []).map((s) => (
                    <span key={s} className={`text-xs px-2 py-0.5 rounded-full font-medium ${STAGE_COLORS[s] || "bg-gray-100 text-gray-600"}`}>
                      {s}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {(vc.sectors || []).slice(0, 4).map((s) => (
                    <span key={s} className="text-xs bg-gray-50 text-gray-600 border border-gray-100 px-2 py-0.5 rounded-full">
                      {s}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-auto space-y-2">
                  {(vc.check_size_display || vc.check_size) && (
                    <div className="text-xs text-gray-500">
                      <span className="font-medium text-gray-700">Check size:</span> {vc.check_size_display || vc.check_size}
                    </div>
                  )}

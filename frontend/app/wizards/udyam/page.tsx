...
const STORAGE_KEY = "udyam_progress";
...
export default function UdyamWizardPage() {
  ...
  const { data, isLoading } = useQuery<WizardData>({
    queryKey: ["wizard-udyam"],
    queryFn: () => api.get("/api/wizards/udyam").then((r) => r.data),
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <div className="max-w-3xl mx-auto">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-stone-900">Udyam Registration Wizard</h1>
              <p className="text-stone-500 text-sm">Register your enterprise as an MSME in minutes.</p>
            </div>
          </div>
          ...
          <div className="glass rounded-2xl p-5 shadow-sm mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-stone-700">Your Udyam Journey</span>
            ...
            <div className="h-3 bg-peach-100/50 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-blue-500 to-coral rounded-full transition-all duration-500"
                ...
                return (
                  <div
                    ...
                    className={`glass border rounded-2xl p-6 shadow-sm transition-all ${
                      isDone ? "border-peach-200/60 bg-peach-50/20" : "border-peach-200/40"
                    }`}
                  >
                    <div className="flex items-start gap-4">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-sm font-bold ${
                        isDone ? "bg-coral text-white" : "bg-peach-50/60 text-stone-500"
                      }`}>
                        {isDone ? "✓" : idx + 1}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-2 text-xs font-medium px-3 py-2 rounded-lg transition-colors ${
                            isDone
                              ? "bg-peach-50/60 text-stone-700 hover:bg-peach-100/60"

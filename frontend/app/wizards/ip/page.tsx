...
const TABS = [
  ...
  { key: "trademark" as const, label: "Trademark", color: "text-blue-600", bg: "bg-blue-50 border-blue-200" },
      ...
      <main className="flex-1 p-8">
        <div className="max-w-3xl mx-auto">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">IP Filing Assistant</h1>
              <p className="text-gray-500 text-sm">Patents, trademarks, and copyrights for Indian startups.</p>
            </div>
          </div>
            ...
            <div className="space-y-5">
              ...
              {section.timeline && (
                <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                  ...
                  <div className="text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-3">{section.timeline}</div>
                </div>
              )}

              {section.fees && section.fees.length > 0 && (
                <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                  <h3 className="font-semibold text-gray-900 mb-4">Fee Schedule</h3>
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {section.fees.map((fee, i) => (
                        <tr key={i}>
                          <td className="py-3 text-gray-600">{fee.item}</td>
                          <td className="py-3 text-right font-medium text-gray-900">{fee.amount}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <ol className="space-y-3">
                    {section.process_steps.map((step, i) => (
                      <li key={i} className="flex gap-3">
                        <span className="w-6 h-6 rounded-full bg-brand-50 border border-brand-100 text-brand-600 text-xs font-bold flex items-center justify-center shrink-0">
                          {i + 1}
                        </span>
              ...
              {section.attorney_finder && (
                <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                  <h3 className="font-semibold text-gray-900 mb-3">Find a Patent/IP Attorney</h3>
